...
void insertData(){
	strcpy(cyno.name, "Cyno"); // fjords, 4p tf, EM/Elec/Crit, 9/10/10
	...
	cyno.critRate = 5/100.0 + 27.6/100.0 + 0.3972; // base + sig + substat
	cyno.critDMG = 50/100.0 + 38.4/100 + 62.2/100.0 + 0.3972; // base + ascension + cd circlet + substat
	cyno.currHP = 100.0/100.0;
	cyno.HB = 0.0/100.0;
	
	strcpy(nahida.name, "Nahida"); // 1.2k atk, 892 em, 175.6 cv | wandering evenstar, 4p dwm, EM/EM/Crit, 9/10/9
	nahida.HP = 10360 * (1 + 0.0992/100) + 4780 + 507.88; // base + substat
	nahida.ATK = (299 + 510) * (1 + 0.0992/100.0) + 311 + 33.08; // base + sig + substat
	nahida.ER = 1 + 0.2204;
	nahida.EM = 115.2 + 165 + 187 + 187 + 187 + 50 + 118.92; // ascension + wandering evenstar + em sands + em gob + dendro resonance + substat
	...
	nahida.critDMG = 50/100.0 + 0.5958; // base + substat
	nahida.currHP = 100.0/100.0;
	nahida.HB = 0.0/100.0;
	
	strcpy(baizhu.name, "Baizhu"); // 38k hp | prototype amber, 4p ohc, HP/HP/HP, 1/9/9
