...
void insertData(){
	strcpy(kokomi.name, "Kokomi"); // 40k hp | sig, 4p ohc, HP/Hydro/Crit, 10/10/10
	kokomi.HP = 13471 * (1 + 46.6/100 + 17.5/100 + 17.5/100 + 10.5/100 + 25.0/100) + 4780; // base + sig + hp sands
	...
	kokomi.critRate = 5.0/100 - 95.0/100 + 31.1/100 + 36.0/100 + 28.0/100 + 36.8/100;
	kokomi.critDMG = 50.0/100 + 4 * 14.0/100 + 14.0/100;
	kokomi.currHP = 100.0/100.0;
	kokomi.HB = 25.0/100.0; // base + sig + hb circlet + 2p ohc
	kokomi.incomingHB = 0.0/100.0;
	
	strcpy(yelan.name, "Yelan"); // 36k hp, 181.3 cv | slingshot, 4p eosf, HP/Hydro/Crit, 1/9/10 
	...
	yelan.ER = 100.0/100 + 20.0/100 + 5.8/100 + 6.5/100; // 2p eosf + substat + substat
	yelan.DMGBonus = 46.6/100.0; // hydro gob
	yelan.critRate = 5/100.0 + 19.2/100.0 + 31.2/100 + 3.1/100.0 + 7.0/100 + 10.9/100 + 11.7/100; // base + ascension + substat + substat
																					  			  // substat + substat
	...
	strcpy(furina.name, "Furina"); // 41k hp, 204.3 cv | hod, 4p gt, HP/HP/Crit, 1/9/9
	furina.HP = 15307 * (1 + (46.6 + 46.6 + 20 + 25)/100) + 4780;
	furina.DMGBonus = 28.0/100.0 + 70/100.0; // a4 passive + 4p gt
	furina.critRate = 5/100.0 + 19.2/100.0 + 36.9/100.0 + 28.0/100; // base + ascension + substat + hod passive
	furina.critDMG = 50/100.0 + 62.2/100.0 + 68.3/100.0 + 46.9/100; // base + cd circlet + substat + hod
...
void viewData(){
	...
	printf("%s CRIT Rate: %.1f%%\n", kokomi.name, kokomi.critRate * 100);
	printf("%s CRIT DMG: %.1f%%\n", kokomi.name, kokomi.critDMG * 100);
